...
class PerformanceTracker {
    ...
    func startTracking(_ operation: String) {
        ...
        await AppLogger.shared.log("Performance tracking started: \(operation)")
    }
    
    func endTracking(_ operation: String) -> TimeInterval? {
        guard let startTime = startTimes[operation] else {
            await AppLogger.shared.logWarning("No start time found for operation: \(operation)")
            return nil
        }
        ...
        await AppLogger.shared.log("Performance tracking completed: \(operation) - \(String(format: "%.3f", duration))s")
        
        // Log warning for slow operations
        if duration > 1.0 {
            await AppLogger.shared.logWarning("Slow operation detected: \(operation) took \(String(format: "%.3f", duration))s")
        }
    ...
    func clearMetrics() {
        ...
        await AppLogger.shared.log("Performance metrics cleared")
    }
...
extension NSObject {
    func measurePerformance<T>(of operation: String, block: () throws -> T) rethrows -> T {
        await PerformanceTracker.shared.startTracking(operation)
        defer { await _ = PerformanceTracker.shared.endTracking(operation) }
        return try block()
    }
