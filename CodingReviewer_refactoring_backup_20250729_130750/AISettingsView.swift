...
struct AISettingsView: View {
        ...
        VStack(spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                .onChange(of: selectedProvider) { oldValue, newValue in
                    ...
                    await AppLogger.shared.debug("Provider changed to: \(providerString)")
                }
            }
    ...
    private func loadAPIKeys() {
        ...
        if let provider = UserDefaults.standard.string(forKey: "selectedAIProvider") {
            ...
            await AppLogger.shared.debug("Loaded provider from UserDefaults: \(provider)")
        } else {
            // Default to OpenAI if no provider is set
            selectedProvider = .openai
            UserDefaults.standard.set("openai", forKey: "selectedAIProvider")
            await AppLogger.shared.debug("Set default provider to openai")
        }
