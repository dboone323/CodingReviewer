...
class IntelligentCodeAnalyzer {
    ...
    func analyzeProject(at projectPath: String) async throws -> ProjectAnalysisResult {
        ...
        var fixableIssues: [CodeIssue] = [];
        
        for filePath in swiftFiles {
            let fileIssues = try await analyzeFile(at: filePath)
            allIssues.append(contentsOf: fileIssues)
            fixableIssues.append(contentsOf: fileIssues.filter { $0.isAutoFixable })
        }
        
        let result = ProjectAnalysisResult(
            analyzedFiles: swiftFiles.count,
            totalIssues: allIssues.count,
            fixableIssues: fixableIssues.count,
            issues: allIssues,
            recommendations: generateRecommendations(from: allIssues)
        )
        
        os_log("Analysis complete: %d files, %d issues found (%d auto-fixable)", 
               log: logger, type: .info, result.analyzedFiles, result.totalIssues, result.fixableIssues)
    ...
    private func analyzeSecurity(lines: [String], filePath: String) -> [CodeIssue] {
        ...
        for (index, line) in lines.enumerated() {
            ...
            if line.contains("AppLogger.shared.log(") && !line.contains("//") {
                issues.append(CodeIssue(
                    ...
                    originalCode: line.trimmingCharacters(in: .whitespaces),
                    suggestedFix: line.replacingOccurrences(of: "AppLogger.shared.log(", with: "os_log("),
                    isAutoFixable: true,
                    category: .security
                ))
            }
...
struct ProjectAnalysisResult {
    ...
    var fixablePercentage: Double {
        return totalIssues > 0 ? Double(fixableIssues) / Double(totalIssues) * 100 : 0
    }
}
