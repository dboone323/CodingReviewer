...
final class CodeReviewViewModel: ObservableObject {
    ...
    func explainIssue(_ issue: AnalysisResult) async {
        ...
        do {
            ...
            let _ = try await aiService.explainCode(codeSnippet, language: selectedLanguage.rawValue)
    ...
    func generateDocumentation() async {
        ...
        do {
            let _ = try await aiService.generateDocumentation(for: codeInput, language: selectedLanguage.rawValue)
    ...
    func showAPIKeySetup() {
        await AppLogger.shared.debug("🧠 [DEBUG] CodeReviewViewModel.showAPIKeySetup() called")
        await AppLogger.shared.debug("🧠 [DEBUG] About to call keyManager.showKeySetup()")
        osLogger.info("🧠 CodeReviewViewModel showAPIKeySetup called")
        keyManager.showKeySetup()
        await AppLogger.shared.debug("🧠 [DEBUG] Completed call to keyManager.showKeySetup()")
    ...
    private func setupAIService() {
        ...
        do {
            let _ = OpenAIService(apiKey: apiKey)
