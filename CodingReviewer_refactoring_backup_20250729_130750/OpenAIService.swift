...
final class OpenAIService: AIServiceProtocol {
    ...
    func generateFix(for issue: CodeIssue) async throws -> CodeFix {
        ...
        let prompt = buildFixPrompt(for: issue)
        let response = try await sendChatRequest(prompt: prompt)
        return try parseFixResponse(response, for: issue)
    }
    ...
    func explainCode(_ code: String, language: CodeLanguage) async throws -> String {
        let prompt = buildExplanationPrompt(code: code, language: language)
        let response = try await sendChatRequest(prompt: prompt)
        return extractContent(from: response)
    }

    func suggestRefactoring(_ code: String, language: CodeLanguage) async throws -> [AISuggestion] {
        let prompt = buildRefactoringPrompt(code: code, language: language)
        let response = try await sendChatRequest(prompt: prompt)
    ...
    private func buildAnalysisPrompt(for request: AIAnalysisRequest) -> String {
        let contextInfo = buildContextInfo(for: request.context)
    ...
    private func buildFixPrompt(for issue: CodeIssue) -> String {
        return """
    ...
    private func buildDocumentationPrompt(code: String, language: CodeLanguage) -> String {
        return """
        Generate comprehensive documentation for the following \(language.aiPromptName) code:

        ```\(language.aiPromptName.lowercased())
        \(code)
        ```

        Please provide documentation that includes:
        1. High-level description of what the code does
        2. Parameters and return values (if applicable)
        3. Usage examples
        4. Any important notes or considerations

        Use appropriate documentation format for \(language.aiPromptName).
        """
    }

    private func buildExplanationPrompt(code: String, language: CodeLanguage) -> String {
        return """
        Explain the following \(language.aiPromptName) code in clear, understandable terms:
        ...
        3. Explanation of any complex patterns or algorithms used
        4. Purpose and benefits of the approach taken

        Make the explanation accessible to developers of varying experience levels.
        """
    }

    private func buildRefactoringPrompt(code: String, language: CodeLanguage) -> String {
        return """
        Suggest refactoring improvements for the following \(language.aiPromptName) code:
    ...
    private func buildContextInfo(for context: AIAnalysisRequest.AnalysisContext?) -> String {
        ...
        if let dependencies = context.dependencies, !dependencies.isEmpty {
            info += "- Dependencies: \(dependencies.joined(separator: ", "))\n"
        }
        if let framework = context.targetFramework {
            info += "- Target Framework: \(framework)\n"
        }
        return info + "\n"
    }
    ...
    private func parseFixResponse(_ response: OpenAIChatResponse, for issue: CodeIssue) throws -> CodeFix {
        let content = extractContent(from: response)
        ...
        let fixData = try JSONDecoder().decode(FixData.self, from: jsonData)
        ...
        return CodeFix(
            ...
            isAutoApplicable: fixData.isAutoApplicable
        )
    }
...
private struct AIAnalysisData: Codable {
...
private struct ComplexityData: Codable {
...
private struct MaintainabilityData: Codable {
    let score: Double
    let rating: String
}

private struct FixData: Codable {
