...
final class APIKeyManager: ObservableObject {
    ...
    func checkAPIKeyStatus() {
        ...
        if !hasGeminiKey {
            await AppLogger.shared.debug("No Gemini API key found")
        }
    }
    ...
    func getGeminiKey() -> String? {
        ...
        if let envKey = ProcessInfo.processInfo.environment["GEMINI_API_KEY"] {
            await AppLogger.shared.debug("Using Gemini API key from environment")
            return envKey
        }
    ...
    func setGeminiKey(_ key: String) throws {
        ...
        await AppLogger.shared.debug("Gemini API key saved successfully")
    }
    ...
    func removeGeminiKey() throws {
        ...
        await AppLogger.shared.debug("Gemini API key removed")
    }
    ...
    func validateGeminiKey(_ key: String) async -> Bool {
        ...
        do {
            ...
            if isValid {
                await AppLogger.shared.debug("Gemini API key validation successful")
            } else {
                await AppLogger.shared.debug("Gemini API key validation failed")
            }
            return isValid
        } catch {
            await AppLogger.shared.debug("Error validating Gemini API key: \(error)")
            return false
        }
    }

    func showKeySetup() {
        await AppLogger.shared.debug("� [DEBUG] APIKeyManager.showKeySetup() called")
        await AppLogger.shared.debug("🔑 [DEBUG] Before change - showingKeySetup: \(showingKeySetup)")
        osLogger.info("🔑 APIKeyManager showKeySetup called, before: \(self.showingKeySetup)")
        showingKeySetup = true
        await AppLogger.shared.debug("🔑 [DEBUG] After change - showingKeySetup: \(showingKeySetup)")
