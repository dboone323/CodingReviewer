...
class MemoryMonitor: ObservableObject {
    ...
    enum MemoryPressure {
        ...
        var threshold: UInt64 {
            switch self {
            case .normal: return 512 * 1024 * 1024 // 512MB
            case .elevated: return 1024 * 1024 * 1024 // 1GB
            case .critical: return 2048 * 1024 * 1024 // 2GB
            }
        }
    ...
    private func updateMemoryUsage() {
        ...
        if usage > MemoryPressure.critical.threshold {
    ...
    private func addWarning(for pressure: MemoryPressure, usage: UInt64) {
        ...
        case .elevated:
            message = "Memory usage is elevated. Consider closing unused features."
        case .critical:
            message = "Critical memory usage detected. Performance may be affected."
        }
...
class ResponseTimeTracker: ObservableObject {
    ...
    func getSuccessRate() -> Double {
        ...
        let successCount = recentResponseTimes.filter { $0.success }.count
