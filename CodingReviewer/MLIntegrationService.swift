...
final class MLIntegrationService: ObservableObject {
    ...
    func analyzeProjectWithML(fileData: [CodeFile] = []) async {
        ...
        do {
            ...
            logger.log("❌ ML analysis failed: \(error)", level: .error, category: .ai)
        }
    ...
    private func loadMLInsights() async {
        ...
        if let content = try? String(contentsOfFile: mlDataPath) {
    ...
    private func loadPredictiveAnalysis() async {
        ...
        if let htmlContent = try? String(contentsOfFile: dashboardPath) {
    ...
    private func loadCrossProjectLearnings() async {
        ...
        if let content = try? String(contentsOfFile: insightsPath) {
