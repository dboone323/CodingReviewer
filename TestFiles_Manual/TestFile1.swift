...
class TestClass1: ObservableObject {
    @Published var data: [String] = []
    
    func loadData() {
        data = Array(1...10).map { "Item $0 from TestFile1" }
    }
    
    func processData() {
        print("Processing data in TestFile1")
        for item in data {
            print("  - \(item)")
        }
    }
...
struct TestView1: View {
    @StateObject private var testClass = TestClass1()
