...
class TestClass5: ObservableObject {
    @Published var data: [String] = []
    
    func loadData() {
        data = Array(1...10).map { "Item $0 from TestFile5" }
    }
    
    func processData() {
        print("Processing data in TestFile5")
        for item in data {
            print("  - \(item)")
        }
    }
...
struct TestView5: View {
    @StateObject private var testClass = TestClass5()
